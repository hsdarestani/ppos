...
HEADER_ALIASES = {
    ...
    'phone_any': ['phone','phone_number','contact_phone','formatted_phone_number','international_phone_number','phonenumber','شماره تماس'],
    ...
    'logo_url': ['logo_url','logo','image','photo','thumbnail','images','لوگو'],
    ...
    'reviews': ['reviews','reviews_count','review_count','reviews count','user_ratings_total','number_of_reviews','تعداد نظر','تعداد نظرات'],
    'latitude': ['latitude','lat','location lat','عرض جغرافیایی'],
    'longitude': ['longitude','lng','lon','long','location lng','طول جغرافیایی'],
    'opening_hours': ['opening_hours','working_hours','hours','business_hours','working hours','ساعات کاری'],
...
def _header_score(values):
    headers = [str(x or '').strip() for x in values]
    mapped = _header_map(headers)
    score = len(mapped)
    if 'business_name' in mapped:
        score += 5
    if 'phone_any' in mapped or 'mobile' in mapped or 'landline' in mapped:
        score += 2
    if 'address' in mapped or 'city' in mapped:
        score += 1
...
def _read_upload_bytes(file_storage):
    """Read Werkzeug uploads into a seekable in-memory buffer.

    Some production uploads arrive as SpooledTemporaryFile objects that do not
    expose the exact seekable/file-like API openpyxl expects. Reading the bytes
    first keeps XLSX parsing consistent across Flask/Werkzeug versions.
    """
...
def _rows_from_upload(file_storage):
    ...
    if filename.endswith('.xlsx'):
        ...
        best_headers = None
        best_score = -1
        for idx, values in enumerate(raw_rows[:50]):
            score, headers = _header_score(values)
            if score > best_score:
                best_index, best_headers, best_score = idx, headers, score
        if best_index < 0 or best_score < 5 or 'business_name' not in _header_map(best_headers or []):
            raise ValueError('ردیف عنوان معتبر در فایل Excel پیدا نشد.')
        headers = best_headers
        for values in raw_rows[best_index + 1:]:
    ...
    for idx, line in enumerate(lines[:50]):
        try:
            ...
            continue
        score, headers = _header_score(headers)
        if score > best_score:
            best_index, best_headers, best_score = idx, headers, score
    if best_index < 0 or best_score < 5 or 'business_name' not in _header_map(best_headers or []):
        raise ValueError('ردیف عنوان معتبر در فایل CSV پیدا نشد.')
...
def register_data_ingest(app, db_path):
    ...
    def import_raw_business_database():
        ...
        try:
            ...
            return redirect(url_for('campaigns_hub'))
        if not first:
            flash('فایل خالی است یا هیچ ردیف داده‌ای بعد از Header ندارد.', 'error')
            return redirect(url_for('campaigns_hub'))
        hm = _header_map(list(first.keys()))
        if 'business_name' not in hm:
            flash('ستون نام کسب‌وکار پیدا نشد. ستون‌هایی مثل Name / title / business_name پشتیبانی می‌شوند.', 'error')
            return redirect(url_for('campaigns_hub'))
        ...
        try:
            ...
            conn.rollback()
            conn.close()
            flash(f'Import متوقف شد و چیزی ناقص ذخیره نشد: {exc}', 'error')
            return redirect(url_for('campaigns_hub'))
        conn.close()
        ...
        flash(
            ...
            f'{no_mobile} مورد بدون موبایل/برای تماس؛ {place_duplicates} Place ID تکراری؛ '
